...
<body>
    ...
    <section id="about" class="about">
		...
		<?php
			...
			$req = mysqli_query($conn, "SELECT * FROM even ORDER BY date_even ASC");
			if ($req)
				{
				    while($data = mysqli_fetch_assoc($req))
                        {
	                            echo "<tr><td><img src= 'img/" . $data['image'] . "' width='150px' heigth='150px'></td><td>" . $data['nom'] . "</td><td>" . $data['description'] . "</td><td>" . $data['date_even'] . "</td><td>" . $data['heure_debut'] . "</td><td>" . $data['ville'] . "</td><td>" . $data['pays'];
                        }
				}
		?>
		</table></div>
